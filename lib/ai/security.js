...
export function checkRateLimit(ip) {
  ...
  record.hour = record.hour.filter(timestamp => now - timestamp < hourWindow);

  if (record.minute.length >= AI_CONFIG.RATE_LIMITS.MAX_REQUESTS_PER_MINUTE) {
    return { 
      success: false, 
      message: "You're sending requests a little too quickly. Please wait a minute and try again." 
    };
  }

  if (record.hour.length >= AI_CONFIG.RATE_LIMITS.MAX_REQUESTS_PER_HOUR) {
    return { 
      success: false, 
      message: "You've reached the maximum requests for this session. Please come back later, or contact Taha directly!" 
    };
  }
