...
export async function handleAIChat(message, history = []) {
  try {
    ...
    // Convert history into a format compatible with sendMessageStream (if needed, but for simplicity we send a single message in the chat context if possible. Actually, the SDK manages history internally if we use chat.sendMessage, but since it's stateless server-side, we must provide the history manually or send it all at once.)
    ...
    // We allow a maximum of 3 tool iterations to prevent infinite loops
    let iterations = 0;
    while (
